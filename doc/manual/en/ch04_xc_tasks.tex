...
\section{Restarting the task}

Restarting the task sets the current turnpoint back to the start point and
resets all the task statistics. \menulabelr{\bmenug{Nav 2/2}\blink\bmenug{Task
restart}} It differs from manually cycling back to the start point by
resetting the task statistics including any turnpoints already rounded.
It can also be used to start a second lap of a competed task.

Note - \bmenug{Task restart} is only active when flying an ordered task. If
the task has been aborted or an alternate turnpoint selected the original task
must be resumed before it can be restarted.
